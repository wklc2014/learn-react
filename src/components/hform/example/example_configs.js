...
export default [{
    ...
    },
    {
        config: {
            ...
            type: 'upload',
            itemApi: {
                rules: [{ required: true, message: '封面图必填' }],
                extra: '这里是帮助提示信息',
            },
            contentApi: {
                action: 'http://30.20.234.75:15000/learn-antd/upload',
            },
            ext: {
                content: null,
                callback: (info) => {
    ...
    },
    {
        config: {
            ...
            ext: {
                service: value => getUser({ name: value }),
                callback: resp => resp.data.map(v => ({ value: v.id, text: v.hitMemo.substring(0, 4) })),
            },
        },
        extMap: {
            label: '疾病名称',
